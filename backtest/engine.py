...
from monitoring.trade_logger import TradeLogger, load_equity
...
class Backtest:
    ...
    def __init__(self, algorithm, bars, settings, out_dir, starting_cash=100_000):
        ...
        self.portfolio = Portfolio(starting_cash, costs["commission_per_share"])
    ...
    def run(self):
        ...
        pending = []
        current_day = None
        day_start_equity = self.portfolio.cash
        ...
        for step, now in enumerate(timeline):
            ...
            if day != current_day:
                ...
                current_day = day
                day_start_equity = self.portfolio.get_value(prices)
            ...
            equity = self.portfolio.get_value(prices)
        ...
        return summarize(self.out_dir, self.portfolio.starting_cash)
    ...
    def _fill(self, orders, now, positions_now, index_of, opens, logger):
        for order in orders:
            ...
            if order["side"] == "buy":
                ...
                qty = min(order["qty"], math.floor(self.portfolio.cash / price))

                if qty <= 0 or not self.portfolio.buy(symbol, qty, price, now):
                    continue
            else:
                price = open_price * (1 - self.cost_rate)
                qty = min(order["qty"], self.portfolio.positions.get(symbol, 0))

                if qty <= 0 or not self.portfolio.sell(symbol, qty, price, now):
                    continue
            ...
            logger.log_trade(
                ...
                qty=qty,
                price=round(price, 4),
                order_id=f"bt-{len(self.portfolio.trades)}",
