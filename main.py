# main.py
#   python main.py download                  cache historical minute bars
#   python main.py backtest [--from --to]    run the strategy on cached data
#   python main.py train-alpha [--until]     walk-forward train the LightGBM alpha
#   python main.py train-rl [--until]        train + evaluate the PPO allocator
#   python main.py live                     trade one session on Alpaca paper
#   python main.py report [--from --to]      graphs from logs/ (or --logs DIR)
#   python main.py demo                      original single-trade example
import argparse
import json
import logging
import sys
...
def setup_logging(name):
    ...
    logging.getLogger("trading.portfolio").setLevel(logging.WARNING)


def cmd_download(settings, args):
    from data.history import update_cache
...
def cmd_backtest(settings, args):
    ...
    if not bars:
        raise SystemExit("No cached data. Run `python main.py download` first.")
...
def cmd_train_alpha(settings, args):
    from ml.train_alpha import train_alpha
...
def cmd_demo(settings, args):
    ...
    print(f"Profit/Loss at +$10: ${portfolio.get_profit_loss({'AAPL': price + 10}):.2f}")
